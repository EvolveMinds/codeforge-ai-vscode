...
const RELEASE_NOTES: Record<string, string> = {
  ...
  '2.13.0': [
    ...
    `2.11 let you configure a report. This one lets you **compose** it. A report is now an ordered list of blocks — KPI tiles, charts, tables, your own text, insights, recommendations, data quality, relationships — and each one is configurable. A chart block carries its **measure, dimension, aggregation, chart type, top-N and sort**, and the pickers list **your actual column names with their inferred types**, because the file has already been read. Anything you leave on *auto* is still chosen for you, so you only pin down what you care about.\n`,
    `### Edit the rendered report directly\n`,
    `Hover any section to **move, duplicate or delete** it, drag it by its grip to reorder, or **double-click any text** — a heading, a caption, a KPI label, a table cell — to fix it in place. These are direct edits to the document: instant, no AI call, and structurally incapable of disturbing the sections you didn't touch. Everything is undoable.\n`,
    `### Refine one block, not the whole document\n`,
    `Select a section and describe the change, and only that card is sent to the model. On a real report that's a few hundred characters instead of several kilobytes — and the model cannot rewrite the rest of your report, because it never sees it.\n`,
    `### A Design tab with no AI in it\n`,
    `Accent colour, light/dark/auto and spacing density apply **instantly** by re-rendering the stylesheet. No regeneration, no waiting, no risk to your content.\n`,
    `### Filters that actually run\n`,
    `Row filters, derived columns, excluded columns, drop-duplicates and row caps are now **executed** — generated as real pandas in the script path — rather than described to a model that may ignore them. Numeric filters work on raw CSV text, so \`revenue >= 100000\` correctly matches \`"$1,284,900"\`. The report discloses the active filters so a filtered figure is never mistaken for a total, and if a filter empties the data the script stops with a clear message naming it instead of failing deep in the analysis.\n`,
    `### Save it as a template, re-run it next month\n`,
    `Any report can be saved as a reusable template capturing its outline, filters and branding. Point it at new data and get the same report. The outline is read back out of the **rendered HTML**, so a template captures what you actually arranged on screen — including the sections you reordered and deleted by hand.\n`,
    `### Export to PDF\n`,
    `Straight from the preview, using the print styles every report already carries: toolbars hidden, cards never split across a page break.\n`,
    `---\n`,
    `## ⇄ Also in this release — Code Convertor (built as 2.12)\n`,
    `2.12 was never published on its own, so it ships here. A new **Code Convertor** mode converts a selection, a file, or a whole folder between **26 languages** — and returns a **fidelity report** saying what mapped 1:1, what was approximated, and what still needs a human. The result opens **side by side with the original** and nothing is written to disk until you approve it. It runs the target language's own parser over the output where that toolchain is installed, sizes every job against the chosen model's real context window rather than truncating silently, and slices oversized files at top-level declarations instead of mid-function.\n`,
    `Full guide: \`docs/CODE_CONVERSION.md\`.\n`,
  ].join('\n'),
  '2.12.0': [
    `## ⇄ Evolve AI 2.12.0 — Code Convertor\n`,
    `### Convert code to another language, and know what you're getting\n`,
    `A new **Code Convertor** mode sits beside Chat, Edit, Create and Analyse. Point it at a selection, a file, or a whole folder, pick from **26 target languages**, and it converts — but the point isn't the conversion, it's what comes back with it.\n`,
    `### A fidelity report, every time\n`,
    `Asking any AI to "convert this to Go" gives you something that *looks* right. The dangerous part is what disappears quietly: a retry loop, a Decimal that became a float64, a library call replaced by a function that doesn't exist. Every conversion now returns a report saying what mapped 1:1, what was **approximated**, what **needs a human**, and how each source dependency was mapped. A conversion that comes back with no report is flagged as such rather than quietly trusted.\n`,
    `### Review before anything is written\n`,
    `The result opens **side by side with the original** — tabs per file, syntax highlighting, the report on its own tab. Nothing touches disk until you approve it, so a conversion you reject leaves no litter. Refine it in plain language (*"return errors instead of panicking, drop the third-party HTTP client"*) and only the affected files are re-emitted; every round is undoable.\n`,
    `### It checks its own work\n`,
    `**Check it parses** runs the target language's own parser over the output when that toolchain is installed — \`gofmt\`, \`javac\`, \`node --check\`, \`php -l\` and a dozen more — and failures can be fed straight back for a repair round. Checks that resolve imports are labelled as such, so a missing local package isn't mistaken for a bad conversion.\n`,
    `### Pick the model for the job\n`,
    `Conversion gets **its own model choice**, separate from the one the rest of the extension uses — pick a big model for a port without changing what your chat runs on. The picker shows each installed model's **real context window**, read from Ollama itself rather than guessed from the name, and tells you which ones can do the job in one pass.\n`,
    `### It refuses to silently truncate your code\n`,
    `Ollama serves a default context window (often 4k) and drops the front of an oversized prompt **without saying so** — you get a confident port of the half it happened to see. Evolve AI now sizes every conversion against the chosen model first, sets the context window explicitly, and tells you up front whether the job fits: *"Too big for one pass (~28k tokens needed, 8k available)"*. Output limits were raised too — every provider was capped at 4096 tokens, enough for a chat reply and not for a converted file.\n`,
    `### Too big? It slices, and says so\n`,
    `A file larger than the model can take is cut at **top-level declarations** — never mid-function — converted part by part with each part told what the earlier ones declared, then stitched back into one file. The report names every rejoined file so you know where the seams are. And before it splits, it offers you the alternative: a model you already have installed that could do it in one pass, or the \`ollama pull\` command for one that would.\n`,
    `### Folders convert as one unit\n`,
    `Converting file-by-file makes each file invent its own vocabulary. A folder goes in one request so shared types and cross-file calls stay consistent, chunked only when the context budget forces it. Choose how faithful (**idiomatic**, **line-by-line** for an auditable diff, or **modernise**) and how free with dependencies (**stdlib only**, **well-known packages**, or **one-for-one with the source**).\n`,
  ].join('\n'),
  '2.11.0': [
  ...
  '2.9.1': [
    ...
    `### Generated scripts that actually run\n`,
    `The Python scripts Evolve AI generates now run on a clean machine against messy data: they **auto-install their own dependencies** (pandas + matplotlib, no more \`ModuleNotFoundError: plotly\`), use \`df.corr(numeric_only=True)\` (no more "could not convert string to float"), coerce stringy-numeric columns like \`"1,000+"\`, and wrap risky steps in try/except so one bad column can't abort the report.\n`,
